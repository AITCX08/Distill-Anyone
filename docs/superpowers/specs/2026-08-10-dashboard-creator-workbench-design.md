...
- 默认保存位置持久化在 Dashboard 本地设置中；单次覆盖位置只以五分钟、会话绑定的令牌提交。任务创建后保存解析后的私有目录，修改默认位置不会改变既有任务。
- 完成态总览改为交付摘要，不再显示下载速度或 ETA。运行态仍只在下载阶段显示传输遥测；作品 ID 收入可展开的技术信息。
- 产物库的公开列表只保存不透明 artifact 标识及标题、类型、大小、时间。保存目录只由 `GET /api/v1/jobs/{job_id}/details` 在本地会话中返回；`POST /api/v1/jobs/{job_id}/reveal-output` 不接受客户端路径，只打开任务已批准的位置。
- 产物白名单同时合并流水线内部 artifact 记录和已经写入输出回执的交付文件，仍限制为任务仓库或该任务已批准的目录。
- 视觉层继续使用 Fluent UI v9 与现有深色工作台主题；操作按钮最小高度为 44px，所有关键控件均保留可见焦点环和小屏单列布局。
